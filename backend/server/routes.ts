...
//           });
//         } else {
//           console.error("❌ Validation failed for lead:", lead, result.error.format());
//         }
//       }
...
// ✅ Local Lead type (with apolloData always required)
type Lead = {
  id: number;
  name: string;
  jobTitle: string;
  ...
  location: string;
  linkedinUrl: string;
  apolloData: any; // always required
  extractedAt: Date;
};
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/leads/batch", async (req, res) => {
    ...
    try {
      ...
      if (!apifyLeads.length) {
        return res.status(200).json({ message: "No leads found. Check Apify run logs." });
      }

      // 4️⃣ Map to our Lead type
      const mappedLeads: Lead[] = apifyLeads.map((item: any, idx: number) => ({
        id: idx + 1,
        name: item.recruiter_name || item.company_name || "Unknown",
        jobTitle: item.title || "N/A",
        ...
        location: item.location || "N/A",
        linkedinUrl: item.job_url || "#",
        apolloData: item ?? {}, // ✅ ensure always object
        extractedAt: new Date(),
      }));

      // 5️⃣ Remove duplicates by linkedinUrl
      const uniqueLeads: Lead[] = Array.from(
        new Map(mappedLeads.map((lead) => [lead.linkedinUrl, lead])).values()
      );
      console.log("✅ Leads after removing duplicates:", uniqueLeads.length);

      // 6️⃣ Validate with safeParse
      const validatedLeads: Lead[] = [];
      for (const lead of uniqueLeads) {
        // ✅ Ensure apolloData is always an object before validation
        const leadToValidate = {
          ...
          location: lead.location,
          linkedinUrl: lead.linkedinUrl,
          apolloData: lead.apolloData || {}, // ✅ Always provide fallback
        };

        const result = insertLeadSchema.safeParse(leadToValidate);
        ...
        if (result.success) {
          validatedLeads.push({
            ...
            id: lead.id,
            extractedAt: lead.extractedAt,
          } as Lead); // ✅ Type assertion for safety
        } else {
          console.error("❌ Validation failed for lead:", lead.name, result.error.format());
        }
      }
  ...
  app.post("/api/export/csv", async (req, res) => {
    try {
      ...
      const headers = ["Name", "Job Title", "Company", "Location", "LinkedIn URL"];
      const csvContent = [
        headers.join(","),
        ...leads.map((lead) =>
          [
